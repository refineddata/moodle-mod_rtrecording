...
$type = $type == 'meeting' ? 'meeting' : 'content';
...
function cp_one($type, $name, $scoid, $type = 'folder', $icon = '', $url = '') {
    if ($type == 'meeting') {
//        $sco = connect_get_sco_by_url($url);
//        if( isset( $sco->archive ) && $sco->archive ){
            echo '<tr><td><a href="#" class="sco-choose" data-name="' . $name . '" data-scoid="' . $scoid . '">' . $name . '</a></td><td>' . $url . '</td></tr>';
//        }
    } elseif( $type == 'content' ) {
        if( $icon == 'archive' ){
            echo '<tr><td>' . $name . '</td><td><a href="#" class="recording-choose">' . $url . '</a></td></tr>';
        }
    } else {
    ...
    <table width="100%">
        <form id="myform">
            ...
            if (!isset($scoid) OR !$scoid) {
                ...
                foreach ($scos as $sco) {
                    cp_one($type, $sco->name, $sco->sco_id, $sco->type, $sco->icon, $sco->url_path);
                }
            } 
?>
        </form>
